...
fn main() {
    ...
    // Windows debug: DO NOT link msvcrtd.
    // The CMake build already handles CRT linking via Config::new().
    // Explicitly linking the debug CRT DLL on top of the release CRT
    // (used by whisper.cpp and other native libs in the same process)
    // causes a CRT mismatch deadlock during heap allocation.
